...
export const CallbacksGrid = () => {
  ...
  const hasCallbackInfo = (message: Message): boolean => {
    return !!(message.user_name || message.user_email || message.user_phone || message.user_company || message.user_callback_message)
  }

  // Helper function to check if a message has any of the specified fields
  const hasRequiredFields = (message: Message): boolean => {
    return !!(message.user_message || message.suggested_question || message.bot_message || message.user_name || message.user_email || message.user_phone || message.user_company || message.user_callback_message)
  }
  ...
  const formatMessages = (
    ...
    const callbackThreads = Object.entries(messages).filter(
      ([_, threadMessages]) => threadMessages.some(hasCallbackInfo) && threadMessages.some(message => !!message.user_email),
    )
